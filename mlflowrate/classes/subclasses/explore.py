...
class DataExplore(BaseData):
    ...
    def status(self):
        ...
        def benchmark(self, date, ytrue, ypred, regression=None):
            """
            ...
            :param regression:
            """
            self.results["benchmark"] = Result(date=date, ytrue=ytrue, ypred=ypred, regression=regression)

        def input_results(self, result_name, date=None, ypred=None, ytrue=None, r2=0.0, rmse=0.0, aic=0.0, aicc=0.0,
                          adj_r2=0.0, regression=None):
            """
            ...
            """
            #       wrapper function
            self.results[result_name] = Result(date, ypred, ytrue, r2, rmse, aic, aicc, adj_r2, regression)
        ...
        def evaluate(self, model, traindataset, testdataset, param_grid, splits=10, testunc=0.15):
            ...
            trainplot(traindate, trainpred, trainy, axs=axs[0])
            evalplot(traindate, testdate, trainy, testy, testpred, testunc=testunc, axs=axs[1])
            display(fig)
            return model_params
        ...
        def plotmodels(self, traindataset, testdataset, modelpreds, testunc=0.15):
            ...
            :param testdataset: testing dataset
            :param modelpreds: model predictions
            :param testunc:
            """
            traindate = traindataset.date
            testdate = testdataset.date
...
class Result:
    def __init__(self, date=None, ypred=None, ytrue=None, r2=0.0, rmse=0.0, regression=None):
        self.date = date
        self.ypred = ypred
        self.ytrue = ytrue
        self.r2 = r2
    ...
    def calc_rmse(self, ytrue, ypred):
        """
        Calculate root-mean-square-error between prediction and actual values
        :param ytrue: target values
    ...
    def plot_residual(self):
        """
        Plot residual graph for predictions
        """
        residual = ypred - ytrue
